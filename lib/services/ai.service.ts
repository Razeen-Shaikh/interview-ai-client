// @ts-nocheck — migrated from Express server; types added incrementally.
import "server-only";

import { GoogleGenerativeAI } from "@google/generative-ai";
import OpenAI from "openai";

import { parseAiJson } from "@/lib/utils/parseAiJson";
...
/** Used when cloud AI is unavailable (quota, billing, or no keys). */
function generateFallbackQuestions(role, level, techStack) {
  const stack =
  ...
  return [
    `As a ${level} ${role}, how would you design and implement a core feature using ${stack}?`,
    `What trade-offs would you weigh when picking libraries or patterns in ${stack}?`,
    `Describe how you would debug a hard production issue involving ${stack}.`,
    `How do you approach testing and code review for ${stack} projects?`,
    `Walk through a technical decision you made with ${stack} and what you would do differently.`,
  ];
}
  ...
  try {
    if (provider === "gemini") {
      try {
        ...
        if (isQuotaError(geminiError) && hasOpenAIKey()) {
          try {
